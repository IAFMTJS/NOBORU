...
values = "\n".join(
    f"    ('{c}', '{r}', '{rn}', '{rl}', {oi}, '{vt}'),"
    for c, r, rn, rl, oi, vt in rows
)

header = """-- Phase 7: Katakana region (Forest Trail)
...
footer = """) as v(character, romaji, row_name, row_label, order_index, variant_type)
where not exists (
...
seed_block = Path("supabase/migrations/20260608180000_hiragana_region.sql").read_text(encoding="utf-8")
seed_start = seed_block.index("do $seed$")
seed = seed_block[seed_start:]
seed = seed.replace("foothills", "forest-trail")
seed = seed.replace("public.hiragana", "public.katakana")
seed = seed.replace("'hiragana'", "'katakana'")
seed = seed.replace("Hiragana", "Katakana")
seed = seed.replace("hiragana", "katakana")
seed = seed.replace("'A Row + ん'", "'A Row + ン'")
seed = seed.replace(
...
seed = seed.replace(
    ...
    "-- Seed katakana curriculum under Forest Trail",
)

output = header + values + footer + seed
Path("supabase/migrations/20260608200000_katakana_region.sql").write_text(output, encoding="utf-8")
