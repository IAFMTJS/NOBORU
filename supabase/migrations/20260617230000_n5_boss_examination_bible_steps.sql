...
declare
  sentinel_id uuid;
begin
  select id into sentinel_id
  from public.trial_templates
  where slug = 'n5-sentinel'
  limit 1;
  ...
  if exists (
    ...
    where ts.trial_template_id = sentinel_id
      and ts.step_kind = 'applied_vocabulary'
  ) then
    return;
  end if;

  delete from public.trial_steps
  where trial_template_id = sentinel_id;
