...
async function loadProfileRecord(userId: string, displayName: string) {
  return profileServerRepository.ensureProfile(userId, displayName);
}

export async function getProfileCoreUncached(): Promise<ProfileViewModel | null> {
  ...
  const profile = await loadProfileRecord(user.id, displayName);
  const stats = PLACEHOLDER_PROFILE_STATS.stats.map((stat) => ({ ...stat }));

  return mapProfileToViewModel(profile, user.email ?? "", stats);
}

export async function getProfileWithStatsUncached(): Promise<ProfileViewModel | null> {
  const user = await authServerRepository.getUser();
  if (!user) {
    return null;
  }
...
/** @deprecated Use getProfileCoreUncached or getProfileWithStatsUncached */
export async function getProfileUncached(): Promise<ProfileViewModel | null> {
  return getProfileWithStatsUncached();
}
