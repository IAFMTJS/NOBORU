...
import { GAME_VOCABULARY_POOL_LIMIT } from "@/features/games/constants/game.constants";
...
class GameContentRepository {
  ...
  async listLearnedVocabulary(userId: string): Promise<VocabularyRow[]> {
    ...
    const cappedIds = ids.slice(0, GAME_VOCABULARY_POOL_LIMIT);
    const rows = await vocabularyRepository.findByIds(cappedIds);
