# Noboru Documentation

Authoritative project documentation. All implementation must reference these documents.

## Governance

| Document | Purpose |
|----------|---------|
| [MASTER_PROMPT.md](./MASTER_PROMPT.md) | Master vision, tech stack, structure |
| [prd.md](./prd.md) | MVP product requirements |
| [mvp-roadmap.md](./mvp-roadmap.md) | Phase-by-phase delivery plan |

## Product

| Document | Purpose |
|----------|---------|
| [vision.md](./vision.md) | Mission and long-term vision |
| [product-strategy.md](./product-strategy.md) | Positioning and scope |
| [personas.md](./personas.md) | Learner personas |
| [user-flows.md](./user-flows.md) | Key user journeys |
| [information-architecture.md](./information-architecture.md) | Routes, nav, content hierarchy |
| [immersive-navigation-system.md](./immersive-navigation-system.md) | Bottom nav as in-world destinations |
| [training-grounds-system.md](./training-grounds-system.md) | Mastery practice separate from journey |

## Design

| Document | Purpose |
|----------|---------|
| [phase-a-visuals-plan.md](./phase-a-visuals-plan.md) | Visual audit, mockup parity, Phase A execution plan |
| [design-system.md](./design-system.md) | Colors, typography, components |
| [art-direction.md](./art-direction.md) | Visual identity, Yama, assets |
| [mascot-expansion-system.md](./mascot-expansion-system.md) | Mountain Fox companion evolution |
| [asset-pipeline.md](./asset-pipeline.md) | Asset creation workflow |
| [asset-registry.md](./asset-registry.md) | Approved asset catalog |

## Technical

| Document | Purpose |
|----------|---------|
| [database-schema.md](./database-schema.md) | Canonical database schema |
| [api-specification.md](./api-specification.md) | API structure and contracts |
| [testing-strategy.md](./testing-strategy.md) | Test pyramid and tooling |
| [deployment.md](./deployment.md) | Vercel, Supabase, CI/CD |

## Educational

| Document | Purpose |
|----------|---------|
| [JWorld/](./JWorld/README.md) | **Living brainstorm** — five JLPT worlds (replaces World Tree concept) |
| [jlpt-content-architecture.md](./jlpt-content-architecture.md) | JLPT curriculum structure |
| [NOBORU LEARNING ARCHITECTURE BIBLE.md](./NOBORU%20LEARNING%20ARCHITECTURE%20BIBLE.md) | Learning progression, pools, mastery, golden content rule |
| [features/daily-challenges/docs/README.md](../features/daily-challenges/docs/README.md) | Bible retention daily challenges (distinct from gamification quests) |

Route: `/daily-challenge` — retention UI (memory maintenance, not progression).
| [content-pipeline.md](./content-pipeline.md) | Content creation workflow |
| [content-strategy.md](./content-strategy.md) | Content ownership and CMS |
| [game-design.md](./game-design.md) | Games and trials |
| [gamification.md](./gamification.md) | Progression, quests, achievements |
| [learning-audit-action-plan.md](./learning-audit-action-plan.md) | Gameplay, pedagogy, and competitive audits (working) |

## Operations

| Document | Purpose |
|----------|---------|
| [admin-panel-spec.md](./admin-panel-spec.md) | Admin panel specification |
